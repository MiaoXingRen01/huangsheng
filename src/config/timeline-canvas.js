...
const TimeSlider = function (elementId, options) {
    ...
    this.start_timestamp = options.initStartTimeStamp;
...
TimeSlider.prototype.mouseupFunc = function (e) {
    ...
    if (_this.g_isMousemove) { //拖动 事件
        ...
        _this.init(_this.start_timestamp, _this.timecell, true)
        _this.g_isMousedown = false;
        var posx = _this.get_cursor_x_position(e)-_this.marginLeft; //鼠标距离 px
        var ms_per_px = (_this.zoom * 3600 * 1000) / _this.canvansW; // ms/px
        _this.returnTime = _this.start_timestamp + posx * ms_per_px;
        _this.ctx.fillStyle = "rgb(64, 196, 255";
        _this.ctx.fillText(_this.changeTime(_this.returnTime), posx, 50);
        _this.drawLine(posx , 0, posx, 33, "rgb(64, 196, 255", 2); //中间播放点时间线
...
TimeSlider.prototype.clearCanvas = function () {
    this.ctx.clearRect(0, 0, this.viewWidth, 150);
}
