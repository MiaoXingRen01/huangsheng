import "@/config/grf/grhtml5-6.8-min.js"
...
//////////////////////////////////////////////////////////////////////////
export function MsgPluginFailed() {
...
export function CreateReport(PluginID, EventParams) {
    var typeid,
        report;
...
export function doInsertPlugin(args) {
...
export function InsertDisplayViewer(args) {
    args.type = "displayviewer";
    doInsertPlugin(args);
}

//创建报表打印显示插件，详细请查看帮助中的 IGRPrintViewer
export function InsertPrintViewer(args) {
    args.type = "printviewer";
    doInsertPlugin(args);
}

//创建报表设计器插件，详细请查看帮助中的 IGRDesigner
export function InsertDesigner(args) {
    args.type = "designer";
    doInsertPlugin(args);
}
...
export function _doAjaxReport(Report, ReportUrl, DataUrl, fun) {
...
export function AjaxReportPrint(Report, ReportUrl, DataUrl, ShowPrintDialog) {
    _doAjaxReport(Report, ReportUrl, DataUrl, function () {
        Report.Print(ShowPrintDialog);
    });
}

export function AjaxReportPrintPreview(Report, ReportUrl, DataUrl, ShowModal) {
    _doAjaxReport(Report, ReportUrl, DataUrl, function () {
        Report.PrintPreview(ShowModal);
    });
}

export function AjaxReportExportDirect(Report, ReportUrl, DataUrl, ExportType, FileName, ShowOptionDlg, DoneOpen) {
    _doAjaxReport(Report, ReportUrl, DataUrl, function () {
        Report.ExportDirect(ExportType, FileName, ShowOptionDlg, DoneOpen);
    });
}

export function AjaxDesignerOpen(designer, LoadReportURL, onreportload) {
...
export function AjaxDesignerSave(designer, SaveReportURL) {
    var str;
...
//从URL处加载报表模板
export function SyncReportLoad(Report, ReportUrl) {
    grplugin_http(ReportUrl, function (xmlhttp) {
        Report.LoadFromStr(xmlhttp.responseText);
    }, false);
}
...
//从URL处加载报表数据
export function SyncReportLoadData(Report, DataUrl) {
    grplugin_http(DataUrl, function (xmlhttp) {
        Report.LoadDataFromAjaxRequest(xmlhttp.responseText, xmlhttp.getAllResponseHeaders()); //加载报表数据
    }, false);
}
...
//从URL处加载记录集数据
export function SyncRecordsetData(Recordset, DataUrl) {
    grplugin_http(DataUrl, function (xmlhttp) {
...
export function SyncChartData(Chart, DataUrl, FirstSeries, AutoSeries, AutoGroup) {
    grplugin_http(DataUrl, function (xmlhttp) {
...
export function SyncChartXYData(Chart, DataUrl, AutoSeries) {
    grplugin_http(DataUrl, function (xmlhttp) {
...
export function SyncChartXYZData(Chart, DataUrl, AutoSeries) {
    grplugin_http(DataUrl, function (xmlhttp) {
...
export function SyncPictureBoxData(PictureBox, Url, Report) { //下一版本PictureBox可以取得Report，则最后一个参数可以不要
...
//////////////////////////////////////////////////////////////////////////
//以下函数是为了兼容以前的写法
export function CreatePrintViewerEx2(PluginID, Width, Height, ReportURL, DataURL, AutoRun, ExParams) {
    doInsertPlugin({
        type: "printviewer",
        id: PluginID,
        width: Width,
...
export function CreateDisplayViewerEx2(PluginID, Width, Height, ReportURL, DataURL, AutoRun, ExParams, OnCreated, OnReportLoaded) {
    doInsertPlugin({
        type: "displayviewer",
        id: PluginID,
        width: Width,
...
export function CreateDesignerEx(Width, Height, LoadReportURL, SaveReportURL, DataURL, ExParams) {
    doInsertPlugin({
        type: "designer",
        width: Width,
        height: Height,
...
export function CreatePrintViewerEx(Width, Height, ReportURL, DataURL, AutoRun, ExParams) {
    CreatePrintViewerEx2("ReportViewer", Width, Height, ReportURL, DataURL, AutoRun, ExParams)
}

export function CreateDisplayViewerEx(Width, Height, ReportURL, DataURL, AutoRun, ExParams) {
    CreateDisplayViewerEx2("ReportViewer", Width, Height, ReportURL, DataURL, AutoRun, ExParams)
}

export function CreatePrintViewer(ReportURL, DataURL) {
    CreatePrintViewerEx("100%", "100%", ReportURL, DataURL, true, "");
}

export function CreateDisplayViewer(ReportURL, DataURL) {
    CreateDisplayViewerEx("100%", "100%", ReportURL, DataURL, true, "");
}

export function CreateDesigner(LoadReportURL, SaveReportURL, DataURL) {
...
export function grplugin_http(url, callback, async, url_params, url_method, cbthis) {
...
export function urlAddRandomNo(url) {
    return url + "?id=" + Math.floor(Math.random() * 10000);
}

//由三原色值合成颜色整数值
export function ColorFromRGB(red, green, blue) {
    return red + green * 256 + blue * 256 * 256;
}

//获取颜色中的红色值，传入参数为整数表示的RGB值
export function ColorGetR(intColor) {
    return intColor & 255;
}

//获取颜色中的绿色值，传入参数为整数表示的RGB值
export function ColorGetG(intColor) {
    return (intColor >> 8) & 255;
}

//获取颜色中的蓝色值，传入参数为整数表示的RGB值
export function ColorGetB(intColor) {
    return (intColor >> 16) & 255;
}
export function insertReportViewer(domId, grfUrl, dataUrl) {
    //创建报表显示器，参数指定其在网页中的占位标签的ID，关联的报表模板URL，关联的报表数据URL
    var reportViewer = rubylong.grhtml5.insertReportViewer(domId, urlAddRandomNo(grfUrl), dataUrl);
    reportViewer.reportPrepared = false; //指定报表生成须要加载报表模板
    reportViewer.dataPrepared = false;   //指定报表生成须要加载报表数据
    //启动报表运行，生成报表
    reportViewer.start();
}