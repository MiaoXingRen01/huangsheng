<template>
  <div :id="$route.name">
    <a-card>
      <template #title>
        <a-row justify="space-between" align="top" class="pt-4">
          <a-form layout="inline">
            <a-form-item>
              <a-input placeholder="账号/姓名/手机号" v-model:value="searchText" />
            </a-form-item>
            <a-form-item label="角色">
              <a-select placeholder="请选择" v-model:value="searchRoleId">
                <a-select-option value>全部</a-select-option>
                <a-select-option v-for="item in roleList" :key="item.id" :value="item.id">{{item.name}}</a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item label="状态">
              <a-select placeholder="请选择" v-model:value="searchStatus">
                <a-select-option value>全部</a-select-option>
            ...
            <a-form-item label="是否删除">
              <a-select placeholder="请选择" v-model:value="searchDelFlag">
                <a-select-option value>全部</a-select-option>
...
export default {
  ...
  async mounted() {
    ...
    if (res.status == 0) {
      this.roleList = res.data.data.filter((t) => t.level == "user");
    }

    const res2 = await app.postAsync(URLS.USER_DEPARTMENT_TREE_LIST, {
      userId: app.userInfo.get().userId,
    });
    if (res2.status == 0) {
  ...
  methods: {
    ...
    async onSubmitEditEntityClick() {
      ...
      let res = await app.postAsync(URLS.ADMIN_USER_SAVE, params);
      if (res.status == 0) {
    ...
    onItemDeleteClick(item) {
      this.$confirm({
        ...
        onOk: async () => {
          app.showLoading();
          const res = await app.postAsync(URLS.ADMIN_USER_DEL, {
            userId: item.userId,
          });
          app.hideLoading();
          if (res.status == 0) {
    ...
    onItemSatusClick(item, status) {
      ...
      this.$confirm({
        ...
        onOk: async () => {
          app.showLoading();
          let res = await app.postAsync(URLS.ADMIN_USER_ENABLE, params);
          app.hideLoading();
          app.showToast(res.message);
          if (res.status == 0) {
            this.onSearchClick();
    ...
    onItemResetPasswordClick(item) {
      this.$confirm({
        ...
        onOk: async () => {
          app.showLoading();
          let res = await app.postAsync(URLS.ADMIN_USER_RESETPWD, {
            userId: item.userId,
          });
          app.hideLoading();
          if (res.status == 0) {
    ...
    onResetClick() {
      this.searchText = this.searchRoleId = "";
      this.searchStatus = this.searchDelFlag = "";
      this.onSearchClick();
    },
    async requestData(showLoading = false) {
      ...
      const res = await app.postAsync(URLS.ADMIN_USER_LIST, params);
      if (showLoading) {
        app.hideLoading();
      }
      if (res.status == 0) {
