...
export default {
  ...
  methods: {
    ...
    async onSubmitEditEntityClick() {
      ...
      app.showLoading();
      const res = await app.postAsync(URLS.ADMIN_DEPARTMENT_SAVE, editEntity);
      app.hideLoading();
      if (res.status == 0) {
    ...
    onItemDeleteClick(item) {
      this.$confirm({
        ...
        onOk: async () => {
          app.showLoading();
          const res = await app.postAsync(URLS.ADMIN_DEPARTMENT_DEL, {
            deptId: item.id,
          });
          app.hideLoading();
          if (res.status == 0) {
    ...
    async requestData(showLoading = false) {
      ...
      let res = await app.postAsync(URLS.ADMIN_DEPARTMENT_LIST, params);
      if (res.status == 0) {
        let data = res.data;
      ...
      res = await app.postAsync(URLS.USER_DEPARTMENT_TREE_LIST, {
        showRoot: "y",
